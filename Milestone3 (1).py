...
##        if(DEBUG):
   ##        print("* Toggled message")
    ...
    def transmit(self):
        ##
        ## transmit - controls the full Morse code message transmission
        ## This function loops through the active message, breaks it into
        ## words, letters, and Morse symbols, then sends the correct state
        ## events for dots, dashes, and pauses.
        ##
        ...
        while not self.endTransmission:
            ...
            ##
            ## Display the active message on our 16x2 screen
            ##
            self.screen.updateScreen(f"Sending: {self.activeMessage}")

            ##
            ## Parse message into individual words
            ##
            wordList = self.activeMessage.split()

            ##
            ## Setup counter to determine if a word pause is needed
            ##
            ...
            for word in wordList:
                ...
                ##
                ## Setup counter to determine if a letter pause is needed
                ##
                lenWord = len(word)
                wordCounter = 1
                ...
                for char in word:
                    ...
                    ##
                    ## Only continue if the character exists in the Morse dictionary
                    ##
                    if morse is not None:

                        ##
                        ## Setup counter to determine if a dot/dash pause is needed
                        ##
                        lenMorse = len(morse)
                        morseCounter = 1

                        ##
                        ## Loop through each dot or dash in the Morse code
                        ##
                        for x in morse:

                            ##
                            ## If the symbol is a dot, transition to the dot state
                            ##
                            if x == ".":
                                self.doDot()
                                self.doDot()
                            ...
                            ##
                            ## If the symbol is a dash, transition to the dash state
                            ##
                            elif x == "-":
                                self.doDash()
                                self.doDash()
                            ##
                            ## If there is another symbol in the same letter,
                    ...
                    ##
                    ## If there is another letter in the word,
                ...
                ##
                ## If there is another word in the message,
                ## pause between words
                ##
...
##
## When the green button is pressed, call processButton
##
